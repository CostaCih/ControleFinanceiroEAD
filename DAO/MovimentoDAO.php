...
class MovimentoDAO extends Conexao{
    public function RealizarMovimento($tipo, $data, $valor, $obs, $categoria, $empresa, $conta){
        if ($tipo == '' || $data == '' || $valor == '' || $categoria == '' || $empresa == '' || $conta == '' ){
           ...
           // Função nativa do PHP para monitorar a segurança dos dados e da ação executada!
        ...
        if($tipoMov == '' || $dtInicio == '' || $dtFinal == ''){
            ...
            $comando_sql = 'SELECT id_movimento,
                                ...
                                ON tb_empresa.id_empresa = tb_movimento.id_empresa
                                    INNER JOIN tb_conta
                                ON tb_conta.id_conta = tb_movimento.id_conta WHERE tb_movimento.id_usuario = ? AND tb_movimento.data_movimento BETWEEN ? AND ?';
    ...
    public function MostrarUltimosMovimentos(){
            ...
            $comando_sql = 'SELECT id_movimento,
                                ...
                                ON tb_empresa.id_empresa = tb_movimento.id_empresa
                                    INNER JOIN tb_conta
                                ON tb_conta.id_conta = tb_movimento.id_conta WHERE tb_movimento.id_usuario = ? ORDER BY tb_movimento.id_movimento DESC LIMIT 10';
